...
def write_excel(rows: list[dict], path: Path,
    ...
    # ── 第2列：欄位標題 ──
    headers = ["來源", "日期", "標題", "發布機關", "連結", "內容", "摘要", "FAQ"]
    ...
    col_widths = [12, 12, 46, 20, 40, 70, 50, 55]
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.cal_start = tkcalendar.DateEntry(self, width=12, date_pattern="yyyy/mm/dd",
                                              font=("微軟正黑體", 10))
        self.cal_start.grid(row=1, column=1, sticky="w", **pad)
        tk.Label(self, text="時間", font=("微軟正黑體", 10)).grid(
            row=1, column=2, sticky="e", padx=(0, 4))
        self.start_time = tk.StringVar(value="00:00")
        ttk.Entry(self, textvariable=self.start_time, width=7,
                  font=("微軟正黑體", 10)).grid(row=1, column=3, sticky="w", padx=(0, 10))
        ...
        self.cal_end = tkcalendar.DateEntry(self, width=12, date_pattern="yyyy/mm/dd",
                                            font=("微軟正黑體", 10))
        self.cal_end.grid(row=2, column=1, sticky="w", **pad)
        tk.Label(self, text="時間", font=("微軟正黑體", 10)).grid(
            row=2, column=2, sticky="e", padx=(0, 4))
        self.end_time = tk.StringVar()
        ttk.Entry(self, textvariable=self.end_time, width=7,
                  font=("微軟正黑體", 10)).grid(row=2, column=3, sticky="w", padx=(0, 10))

        # 輸出目錄
        tk.Label(self, text="輸出目錄", font=("微軟正黑體", 10)).grid(
            row=3, column=0, sticky="e", **pad)
        self.out_dir = tk.StringVar(value=str(OUT_DIR))
        ttk.Entry(self, textvariable=self.out_dir, width=38,
                  font=("微軟正黑體", 9)).grid(row=3, column=1, columnspan=3,
                                               sticky="we", padx=(0, 10), pady=6)

        # AI 設定區
        sep = ttk.Separator(self, orient="horizontal")
        sep.grid(row=4, column=0, columnspan=4, sticky="we", padx=10, pady=(4, 0))
        ...
        self.ai_chk.grid(row=5, column=0, columnspan=2, sticky="w", padx=12, pady=4)

        tk.Label(self, text="模型", font=("微軟正黑體", 10)).grid(
            row=5, column=2, sticky="e", padx=(0, 4))
        ...
        self.model_combo.grid(row=5, column=3, sticky="w", padx=(0, 10))

        self.ollama_status = tk.Label(self, text="● Ollama 檢查中…",
                                      font=("微軟正黑體", 9), fg="gray")
        self.ollama_status.grid(row=6, column=0, columnspan=4, sticky="w", padx=14, pady=(0, 4))

        sep2 = ttk.Separator(self, orient="horizontal")
        sep2.grid(row=7, column=0, columnspan=4, sticky="we", padx=10, pady=(0, 4))

        # 開始 / 停止按鈕
        btn_frame = tk.Frame(self)
        btn_frame.grid(row=8, column=0, columnspan=4, pady=6)
        ...
        self.progress.grid(row=9, column=0, columnspan=4, padx=10, pady=(0, 4))

        # 日誌
        tk.Label(self, text="執行紀錄", font=("微軟正黑體", 10, "bold")).grid(
            row=10, column=0, columnspan=4, sticky="w", padx=12)
        frame = tk.Frame(self)
        frame.grid(row=11, column=0, columnspan=4, padx=10, pady=(0, 12), sticky="nsew")
    ...
    def _set_defaults(self):
        ...
        self.cal_start.set_date((now - timedelta(days=1)).date())
    ...
    def _save(self, rows: list[dict], start_dt: datetime, end_dt: datetime):
        ...
        out_path = Path(self.out_dir.get()) / f"news_{stamp}.xlsx"
        try:
